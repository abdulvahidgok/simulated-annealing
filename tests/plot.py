...
from optimization_problems import tsp
...
class TSPSolver(tsp.TSPSolver):

    def __init__(self, plot_coords=False, *args, **kwargs):
        self.plot_coords = plot_coords
        self.figure, self.ax, self.lines = None, None, None
        super(TSPSolver, self).__init__(*args, **kwargs)
    ...
    def thermal_equilibrium_achievement(self):
        super(TSPSolver, self).thermal_equilibrium_achievement()
        if self.plot_coords:
